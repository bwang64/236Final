...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_hid: int,
        ...
        # nhead = size0 * size1
        encoder_layers = TransformerEncoderLayer(embed_dim, nhead, d_hid, dropout, norm_first=True, batch_first=True)
        self.transformer_encoder = TransformerEncoder(encoder_layers, nlayers)
        self.reverse_patch_embed = ReversePatchEmbed()
        self.sin_pos = PositionalEncoding(d_model=embed_dim, dropout=0, max_len=nhead)
        #self.embedding = nn.Embedding(ntoken, d_model)
        #self.d_model = d_model
        #self.linear = nn.Linear(d_model, ntoken)
        self.pos_embed = nn.Parameter(torch.zeros(1, nhead, embed_dim))
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        # print(f"?????523 First parameter name: {name}")
