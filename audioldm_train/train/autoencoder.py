...
def main(configs, exp_group_name, exp_name):
    ...
    loader = DataLoader(
        subset, batch_size=batch_size, num_workers=8, pin_memory=True, shuffle=True, persistent_workers=True
    )#dataset -> subset
    ...
    try:
        ...
        #print("\n\n\n\nthere exists config", config_reload_from_ckpt)
    except:
    ...
    #config_reload_from_ckpt = configs["reload_from_ckpt"]
    #print("\n\n\nhere!!",config_reload_from_ckpt)

    if len(os.listdir(checkpoint_path)) > 0:
        print("\n\n\n\nLoad checkpoint from path: %s" % checkpoint_path)
    ...
    trainer = Trainer(
        ...
        limit_train_batches=0.5, # control the iteration number of training
    ...
    # 手动更新调度器
    # for _ in range(trainer.current_global_step):
    #     trainer.scheduler.step()

    # TRAINING
    trainer.fit(model, loader, val_loader, ckpt_path=resume_from_checkpoint)#, ckpt_path=resume_from_checkpoint)
...
if __name__ == "__main__":
    ...
    #torch.distributed.init_process_group(backend='gloo',init_method='env://', rank=int(os.environ["RANK"]), world_size=int(os.environ["WORLD_SIZE"]))
