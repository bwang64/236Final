...
class PatchEmbed(nn.Module):
    def __init__(self, img_size0=256, img_size1=16, patch_size=16, stride=16, in_chans=512, embed_dim=768):
        super().__init__()
...
class ReversePatchEmbed(nn.Module):
    def __init__(self, in_channels=768, out_channels=512, kernel_size=(16, 16), stride=(16, 16)):
        super().__init__()
    ...
    def forward(self, x):
        x = self.reverproj(x.transpose(1, 2).unsqueeze(3))
        return x
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_hid: int,
                 nlayers: int, dropout: float = 0.5, img_size0=256, img_size1=16, patch_size=16, stride=16, in_chans=512, embed_dim=768, nhead = 8):
        super().__init__()
        self.model_type = 'Transformer'
        #self.pos_encoder = PositionalEncoding(d_model, dropout)
        self.patch_embed = PatchEmbed(img_size0=img_size0, img_size1=img_size1, patch_size=patch_size, stride=stride, in_chans=in_chans, embed_dim=embed_dim)
        size0 = (img_size0 - patch_size) // stride + 1
        size1 = (img_size1 - patch_size) // stride + 1
        max_len = size0 * size1  #revision place 1
        encoder_layers = TransformerEncoderLayer(embed_dim, nhead, d_hid, dropout, norm_first=True, batch_first=True, activation = "gelu") # revision place 1
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        #print("!!!!!!!!!!!!!517 src: ", src, "shape: ", src.shape)
        # if src_mask is None:
        ...
        name, param = next(iter(self.transformer_encoder.named_parameters()))
        print(f"?????525 First parameter name: {name}")
        print(f"Shape: {param.size()}")
        print(param)
        #print("!!!!!!!!!!!!!528 src: ", output, "shape: ", output.shape)
...
class Encoder(nn.Module):
    def __init__(
        ...
        **ignore_kwargs,
        ...
        self.encoder = TransformerModel(d_hid=4*768, nlayers=4, dropout=0.1)
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        self.decoder = TransformerModel(d_hid=4*768, nlayers=4, dropout=0.1)
