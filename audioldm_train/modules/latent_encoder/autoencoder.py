...
class AutoencoderKL(pl.LightningModule):
    def __init__(
        ...
        subband=1,
        sampling_rate=16000,
        ckpt_path= None, #"C:/Users/yingke/Documents/yingke/stanford/y1q1/CS236/AudioLDM-training-finetuning-main/data/checkpoints/vae_mel_16k_64bins.ckpt", #None,
        reload_from_ckpt=None, #"C:/Users/yingke/Documents/yingke/stanford/y1q1/CS236/AudioLDM-training-finetuning-main/data/checkpoints/vae_mel_16k_64bins.ckpt",
        ignore_keys=[],#["encoder.conv_in.weight"],
        image_key="fbank",
        ...
        if not self.reloaded and self.reload_from_ckpt is not None:
            print("\n\n\n--> Reload weight of autoencoder from %s" % self.reload_from_ckpt)
    ...
    #     if self.trainer.global_step in [100, 200, 300]:
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log_images(inputs, waveform=waveform) #generate many samples.
        ...
        aeloss, log_dict_ae, recloss, gloss = self.loss(
            inputs=inputs,
            reconstructions=reconstructions,
            posteriors=posterior,
            waveform=waveform,
        ...
        for param_group in self.trainer.optimizers[0].param_groups:
            param_group['lr'] -= 0.00005*param_group['lr'] #0.00005
            if param_group['lr'] < 1e-08:
                param_group['lr'] = 1e-08
    ...
    #         # if param_group['lr'].last_epoch < 60000:
    #         #     scheduler.step()
    ...
    def configure_optimizers(self):
        ...
        opt_ae = torch.optim.Adam(params, lr=lr, betas=(0.5, 0.9))#, weight_decay = 0.1
        ...
        opt_disc = torch.optim.Adam(disc_params, lr=lr, betas=(0.5, 0.9))#, weight_decay = 0.1
        ...
        def lambda_function():
            if self.global_step < 10:
                ...
                return 0.67
            elif self.global_step < 30:
                return 0.5
            else:
                return 0.25

        #scheduler_ae = LambdaLR(opt_ae, lr_lambda=lambda_function)
        scheduler_ae = LinearLR(opt_ae, start_factor = 1, end_factor = 1, total_iters = 60000)
        #print("!!!!!!!scheduler_ae: ", scheduler_ae)
        #scheduler_disc = LambdaLR(opt_disc, lr_lambda=lambda_function)
        scheduler_disc = LinearLR(opt_disc, start_factor = 1, end_factor = 1, total_iters = 60000)
        #scheduler_ae = MultiStepLR(opt_ae, milestones=[600, 1500, 2400], gamma=0.3)
        #scheduler_disc = MultiStepLR(opt_disc, milestones=[600, 1500, 2400], gamma=0.3)
