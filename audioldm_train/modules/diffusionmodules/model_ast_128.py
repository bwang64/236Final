...
class ReversePatchEmbed(nn.Module):
    def __init__(self, in_channels=768, out_channels=256, kernel_size=(16, 16), stride=(16, 16)):
        super().__init__()
    ...
    def forward(self, x):
        x = self.reverproj(x.transpose(1, 2).reshape(2, 768, 64, 8))
        return x
...
class AstModel(nn.Module):
    def __init__(self, d_hid=2048,
        ...
        super().__init__()
        self.model = ASTModel.from_pretrained("MIT/ast-finetuned-audioset-16-16-0.442", max_length=1024, num_mel_bins=128, ignore_mismatched_sizes=False)
        self.reverse_patch_embed = ReversePatchEmbed()
        self.norm = torchvision.transforms.Normalize(0, 0.5, inplace=False)
        self.conv1 = torch.nn.Conv2d(
                1, 1, kernel_size=3, stride=1, padding=1
            )
        self.conv2 = torch.nn.Conv2d(
                256, 256, kernel_size=(1,3), stride=(1,2), padding=0
            )
    ...
    def forward(self, x) -> Tensor:
        ...
        """
        x = torch.nn.functional.interpolate(x, size=(1024, 128), mode="nearest")
        x = self.conv1(x)
        x = self.norm(x).squeeze()
        x = self.model(x).last_hidden_state[:, 2:, :]
        #print(x.shape)
        output = self.reverse_patch_embed(x)
        pad = (0, 1, 0, 0)
        output = torch.nn.functional.pad(output, pad, mode="constant", value=0)
